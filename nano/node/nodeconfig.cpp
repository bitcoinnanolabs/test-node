...
const char * default_live_peer_network = "peers.bitcoinnano.org";
}
...
external_address (boost::asio::ip::address_v6{}.to_string ())
{
	...
	{
		case nano::nano_networks::btco_test_network:
			enable_voting = true;
			preconfigured_representatives.push_back (network_params.ledger.genesis_account);
			break;
		case nano::nano_networks::btco_beta_network:
			...
			break;
		case nano::nano_networks::btco_live_network:
	...
	switch (version_a)
	{
		...
		case 15:
		{
			...
			peers_l.array_entries<std::string> ([&peers](std::string entry) {
				...
				else if (entry == "peering.bitcoinnano.org")
				{
