...
int main (int argc, char * const * argv)
{
	...
	description.add_options ()
		...
		("debug_output_last_backtrace_dump", "Displays the contents of the latest backtrace in the event of a btco_node crash")
		...
		("debug_profile_process", "Profile active blocks processing (only for btco_test_network)")
		("debug_profile_votes", "Profile votes processing (only for btco_test_network)")
		("debug_profile_frontiers_confirmation", "Profile frontiers confirmation speed (only for btco_test_network)")
	...
	if (ec == nano::error_cli::unknown_command)
	{
		...
		{
			if (boost::filesystem::exists ("btco_node_backtrace.dump"))
			{
				// There is a backtrace, so output the contents
				std::ifstream ifs ("btco_node_backtrace.dump");
		...
		{
			nano::network_constants::set_active_network (nano::nano_networks::btco_test_network);
		...
		else if (vm.count ("debug_random_feed"))
		{
			 ...
			 *
			 *   ./btco_node --debug_random_feed | dieharder -a -g 200
			 */
			nano::raw_key seed;
			for (;;)
			{
		...
		else if (vm.count ("debug_sys_logging"))
		{
			...
			{
				std::cerr << "The event log requires the HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Services\\EventLog\\Btco\\Btco registry entry, run again as administator to create it.\n";
				return 1;
			}
#endif
			auto inactive_node = nano::default_inactive_node (data_path, vm);
		...
		{
			std::cout << "Version " << BTCO_VERSION_STRING << "\n"
