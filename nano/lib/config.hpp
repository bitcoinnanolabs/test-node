...
*/
const char * const BTCO_VERSION_STRING = xstr (TAG_VERSION_STRING);
const char * const BTCO_MAJOR_VERSION_STRING = xstr (MAJOR_VERSION_STRING);
const char * const BTCO_MINOR_VERSION_STRING = xstr (MINOR_VERSION_STRING);
const char * const BTCO_PATCH_VERSION_STRING = xstr (PATCH_VERSION_STRING);
const char * const BTCO_PRE_RELEASE_VERSION_STRING = xstr (PRE_RELEASE_VERSION_STRING);
...
public:
	...
	static bool set_active_network (std::string network_a)
	{
		...
		if (network_a == "live")
		{
			active_network = nano::nano_networks::btco_live_network;
		}
		else if (network_a == "beta")
		{
			active_network = nano::nano_networks::btco_beta_network;
		}
		else if (network_a == "test")
		{
			active_network = nano::nano_networks::btco_test_network;
		}
		else
		{
			error = true;
	...
	{
		return current_network == nano_networks::btco_live_network;
	}
	bool is_beta_network () const
	{
		return current_network == nano_networks::btco_beta_network;
	}
	bool is_test_network () const
	{
		return current_network == nano_networks::btco_test_network;
	}
