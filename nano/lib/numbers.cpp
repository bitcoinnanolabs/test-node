...
bool nano::public_key::decode_account (std::string const & source_a)
{
	...
	if (!error)
	{
		...
		auto btco_prefix (source_a[0] == 'b' && source_a[1] == 't' && source_a[2] == 'c' && source_a[3] == 'b' && (source_a[4] == '_' || source_a[4] == '-'));
		error = (bcb_prefix && source_a.size () != 64) || (btco_prefix && source_a.size () != 65);
		if (!error)
		{
			if (bcb_prefix || btco_prefix)
			{
