...
your_account_label (new QLabel ("Your Btco account:")),
...
wallet (wallet_a)
{
	...
	version = new QLabel (boost::str (boost::format ("%1% %2% network") % BTCO_VERSION_STRING % network).c_str ());
...
wallet (wallet_a)
{
	...
	QObject::connect (refresh, &QPushButton::released, [this]() {
		...
		if (!account.decode_account (account_line->text ().toStdString ()))
		{
			...
			auto final_text (std::string ("Balance (BTCO): ") + wallet.format_balance (balance.first));
