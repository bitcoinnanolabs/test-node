...
if [ -n "$DOCKER_PASSWORD" ]; then
    ...
    if [[ "$TRAVIS_BRANCH" == "master" || "$TRAVIS_BRANCH" == "docker_cache" ]] && [[ "${TRAVIS_BUILD_STAGE_NAME}" =~ 'Build' ]]; then
        ci_image_name="bitcoinnano/btco-env:$TRAVIS_JOB_NAME"
    ...
    if [[ "$TRAVIS_BUILD_STAGE_NAME" == "Master_beta_docker" ]]; then
        ...
        "$scripts"/custom-timeout.sh 30 docker build --build-arg NETWORK=beta --build-arg CI_BUILD=true --build-arg TRAVIS_TAG="$TRAVIS_TAG" -f docker/node/Dockerfile -t bitcoinnano/btco-beta:master --cache-from bitcoinnano/btco-beta:master .
        "$scripts"/custom-timeout.sh 30 docker push bitcoinnano/btco-beta:master
        ...
        if [[ "$TRAVIS_JOB_NAME" =~ "live" ]]; then
            ...
            cached="--cache-from=bitcoinnano/btco-beta:master"
            docker pull bitcoinnano/btco-beta:master
        fi

        docker_image_name="bitcoinnano/btco${network_tag_suffix}"
