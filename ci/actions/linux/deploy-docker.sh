...
if [ -n "$DOCKER_PASSWORD" ]; then
    ...
    if [[ "$GITHUB_WORKFLOW" = "Develop" ]]; then
        "$scripts"/custom-timeout.sh 30 docker push "bitcoinnano/btco-env:base"
        "$scripts"/custom-timeout.sh 30 docker push "bitcoinnano/btco-env:gcc"
        "$scripts"/custom-timeout.sh 30 docker push "bitcoinnano/btco-env:clang"
